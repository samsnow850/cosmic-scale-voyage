...
const Index = () => {
  ...
  const filteredPlanets = planetsData.filter(planet => planet.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="min-h-screen bg-black overflow-auto">
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="flex flex-wrap gap-4 justify-center items-center mb-6">
          <div className="relative">
            ...
            <Input placeholder="Search planets..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-gray-800 border-gray-600 text-white w-48" />
          </div>
          
          <Button onClick={() => setIsAnimating(!isAnimating)} className="bg-gray-800 border-gray-600 hover:bg-gray-700 text-white border-2" size="sm">
            {isAnimating ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
          </Button>
          
          <Button onClick={() => setShowOrbitalPaths(!showOrbitalPaths)} className="bg-gray-800 border-gray-600 hover:bg-gray-700 text-white border-2" size="sm">
            {showOrbitalPaths ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            Orbits
          </Button>
          
          <Button onClick={() => setShowLightVisualization(!showLightVisualization)} className="bg-gray-800 border-gray-600 hover:bg-gray-700 text-white border-2" size="sm">
            <Zap className="w-4 h-4" />
            Light Speed
          </Button>
          
          <Button onClick={() => setShowSizeComparison(!showSizeComparison)} className="bg-gray-800 border-gray-600 hover:bg-gray-700 text-white border-2" size="sm">
            Size Mode
          </Button>
        </div>

        {/* Search Results */}
        {searchTerm && <div className="max-w-md mx-auto mb-4">
            <div className="bg-gray-800 rounded-lg p-2">
              {filteredPlanets.map(planet => <button key={planet.name} onClick={() => jumpToPlanet(planet.name)} className="w-full text-left p-2 hover:bg-gray-700 rounded text-white">
                  {planet.name}
                </button>)}
            </div>
          </div>}
        ...
        <div className="max-w-md mx-auto mb-6">
          <div className="flex items-center mb-2">
            ...
            <Slider defaultValue={[1]} min={0.1} max={1} step={0.05} onValueChange={value => setScaleMultiplier(value[0])} className="mx-2" />
      ...
      <div ref={scrollContainerRef} className="relative z-10 h-96 overflow-x-auto overflow-y-hidden">
        ...
        width: `${getPosition(3010) + 400}px`
          ...
          {/* Planets */}
          {planetsData.map(planet => <Planet key={planet.name} {...planet} position={getPosition(planet.distanceInches)} onClick={() => {
          setSelectedPlanet(planet);
          playPlanetSound(planet.name);
        }} onAddToComparison={() => addToComparison(planet)} isAnimating={isAnimating} timeSpeed={timeSpeed} showSizeComparison={showSizeComparison} showLightVisualization={showLightVisualization} />)}

          {/* Distance markers */}
          <div className="absolute bottom-4 left-0 right-0 flex items-center text-gray-500 text-xs">
            {[100, 500, 1000, 2000, 3000].map(distance => <div key={distance} className="absolute transform -translate-x-1/2" style={{
            left: `${getPosition(distance)}px`
          }}>
      ...
      {/* Planet Overlay */}
      <PlanetOverlay planet={selectedPlanet} onClose={() => setSelectedPlanet(null)} onAddToComparison={addToComparison} />
